...
function setRandomImage(element) {
  ...
  element.src = willSmithImages[randomImg];
}

const imgs = document.getElementsByTagName("img");
for (let i = 0; i < imgs.length; i++) {
  setRandomImage(imgs[i]);
}

const anchors = document.getElementsByTagName("a");
for (let i = 0; i < anchors.length; i++) {
  const anchorImgs = anchors[i].getElementsByTagName("img");
  for (let j = 0; j < anchorImgs.length; j++) {
    setRandomImage(anchorImgs[j]);
  }
}
